...
//      $('#clickfilter').hide();
  ...
  $('#selecticon').click(function(){
    if ($('#feedback').css('display') === "none"){
      ...
      $("#navbar").animate({'padding-bottom': '5rem'}, 300, function() {
      });
      $("#container").animate({'padding-top': '4.5rem'}, 300, function() {
        $('#feedback, #alert, #message, #aboutus').css({'display': "block"});
      });
      $("#profilepanel").animate({'padding-top': '4.5rem'}, 300, function() {
        $('#feedback, #alert, #message, #aboutus').css({'display': "block"});
      });
      $("#group-img").animate({'top': '7.5rem'}, 300, function() {
        $('#feedback, #alert, #message, #aboutus').css({'display': "block"});
      });
      $("#img-cards").animate({'top': '20rem'}, 300, function() {
      });
    } else {
      $('#navbar').css({'border-radius': '0'});
      $('#feedback, #alert, #message, #aboutus').css({'display': "none"});
      $("#navbar").animate({'padding-bottom': '0'}, 300, function() {
      });
      $("#container").animate({'padding-top': '0'}, 300, function() {
      });
      $("#profilepanel").animate({'padding-top': '0'}, 300, function() {
      });
      $("#group-img").animate({'top': '2rem'}, 300, function() {
      });
      $("#img-cards").animate({'top': '14rem'}, 300, function() {
      });
    }
  })
})
