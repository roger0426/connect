...
$(document).ready(function () {
  ...
  $("#insertbox").click(function () {
    ...
    $('input[type=text]').val('');
    $('textarea').val('');
    $('#id_image').val(''); // change "date" filed from text field to date filed

    $("#id_event_date").attr("onfocus", "(this.type='date')").attr("onfocusout", "(this.type='text')");
    $("#id_due_date").attr("onfocus", "(this.type='date')").attr("onfocusout", "(this.type='text')");
    $('body').css({
      'overflow': 'hidden'
    });
    $("#filter1").show();
  ...
  $("#exitbtn2, #filter1").click(function () {
    ...
    $("#eventcreatewindow").animate({
      ...
      $('input[type=text]').val('');
      $('textarea').val('');
      $('#id_image').val(''); //$("#eventcreatewindow").css({'content-visibility': 'hidden'});
    ...
    $("#comment-window").animate({
      ...
      $('input[type=text]').val(''); //$("#comment-window").css({'content-visibility': 'hidden'});
  ...
  if (searchParams.includes('event')) {
    console.log('here'); //$('#eventwindow').show();
  ...
  var intervalId = window.setInterval(function () {
    ...
    $('#insertbar').toggle();
  }, 450); //  //每次click都ajax去更新每個eventpost的參與數、愛心數
...
function event_handler(URL, user_id, CSRF) {
  ...
  $.ajax({
    ...
    success: function success(data) {
      ...
      if (data.comments != undefined) {
        data.comments.forEach(function (item, i) {
          ...
          $('#' + clone_id).show(); //$('#' + clone_id + " img").attr('src', data.author_img_url);
      ...
      if (data.likes != undefined) {
        data.likes.forEach(function (item, i) {
          ...
          var str = "<a href=\"/profile/" + item.id + "\"><img class=\"member interested\" src=\"https://res.cloudinary.com/connect-universe/image/upload/v1/" + item.img + "\"></img></a>";
      ...
      if (data.particitants != undefined) {
        data.particitants.forEach(function (item, i) {
          ...
          var str = "<a href=\"/profile/" + item.id + "\"><img class=\"member participant\" src=\"https://res.cloudinary.com/connect-universe/image/upload/v1/" + item.img + "\"></img></a>";
          $("#eventoperate #member").prepend(str);
        });
      }
    ...
    complete: function complete(data) {
      ...
      $("#filter1").show(); //      $("#eventwindow").css({'opacity': 1});
...
function like_handler(URL, event_id, CSRF) {
  ...
  $.ajax({
    ...
    success: function success(data) {
      if (data.status == 200) {
        console.log('success');
        var img_url = data.user_img_url;
        var hostname = $(location).attr('hostname');

        if (data.add) {
          var profile_url = hostname + '/profile/' + data.user_id;
          var str = "<a herf='" + profile_url + "'>\
          <img class='member interested' src= " + img_url + "></a>"; //console.log(str);
          ...
          var user_selector = "#member a img[src='" + img_url + "']"; //console.log(user_selector)
...
function duplicate_multi(duplicateID) {
  ...
  clone.id = duplicateID + '_new' + ++i; // or clone.id = ""; if the divs don't need an ID
...
function duplicate(duplicateID) {
  ...
  clone_id = duplicateID; // or clone.id = ""; if the divs don't need an ID

  original.parentNode.insertBefore(clone, original.parentNode.firstChild); //$("eventpost").parent.prepend();
}
