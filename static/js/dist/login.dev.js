...
$(document).ready(function () {
  console.log('in');
  $('#sign-up').on('click touch', function () {
    $('#login-panel').hide();
    $('#sign-up-panel').show();
    console.log('here');
  });
  ...
  $('#sign-up-btn').on('click touch', function () {
...
var intervalId = window.setInterval(function () {
  ...
  if (!$('#terms').is(':checked') || $('#double-pwd').val().length == 0 || $('#email').val().length == 0 || $('#sid').val().length == 0) {
    $('#sign-up-btn').prop('disabled', true);
  } else {
    $('#sign-up-btn').prop('disabled', false);
  }
